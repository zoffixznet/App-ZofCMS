package App::ZofCMS::Plugin::RandomPasswordGeneratorPurePerl;
...
sub _do {
    ...
    for ( 1 .. $conf->{pass_num} ) {
        my @chars = @{ $conf->{chars} || ['0'] };
        @chars = '0'
            unless @chars;

        my $password = '';

        while ( $conf->{length} > length $password ) {
...
B<This plugin is is a drop-in replacemeant of> L<App::ZofCMS::Plugin::RandomPasswordGenerator>
...
=head2 C<plug_random_password_generator_pure_perl>
...
B<Optional>. Takes an I<arrayref> as a value. Elemeants of this arrayref must be characters;
...
be autovivified); why? see C<key> argumeant below.
B<Defaults to:> C<d>
...
Examples below assume that C<cell> argumeant is set to C<d> and C<key> argument is set
...
refered to by> C<key> and C<cell> plugin's argumeants; in other words, if C<cell> is
...
an I<arrayref> first elemeant of which will be the generated password and second element will
...
elemeant of an arrayref instead and output value will be an arrayref.
