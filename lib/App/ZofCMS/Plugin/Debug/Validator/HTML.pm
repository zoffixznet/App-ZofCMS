...
The C<t_name> argumeant specifies the name of that key. See C<HTML::Template VARIABLES> section
...
is set to a true value. This is to speed up normal developmeant process (because
...
B<Optional>.
Plugin accesses a W3C markup validator. The C<validator_uri> argumeant takes a URI pointing
...
Generally you don't need to touch C<address> argumeant as the plugin will do its black magic
...
argumeant that takes a string that is the URI to the page you wish to validate.
...
C<t_name> argumeant, which defaults to C<plug_val_html>.
...
of that variable is contructed by appending C<_link> to the C<t_name> argumeant, thus
